...
class Params:
    ...
    LABEL_SMOOTH = 0.1
    DROPOUT_RATE = 0.4
    # Save paths (updated for DualStream)
    WEIGHT_SAVE_DIR = "./DualStream_Weights"
...
# -------------------------- 3. Dataset loading (designed for class balance) --------------------------
...
# -------------------------- 5. Loss function (weighted as required) --------------------------
...
# -------------------------- 6. Evaluation and testing functions (metrics display) --------------------------
@torch.no_grad()
...
def test_best_model():
    ...
    # Use DualStream model for testing
    model = DualStreamSADMModel(
        feature_dim=256,
        num_classes=2,
    ...
    # load checkpoint (must be a DualStream checkpoint)
...
# -------------------------- 7. Main training function (meets all requirements) --------------------------
def main():
    os.makedirs(Params.WEIGHT_SAVE_DIR, exist_ok=True)
    ...
    model = DualStreamSADMModel(
        feature_dim=256,
        num_classes=2,
        pretrained_spatial=not Params.START_FROM_BEST
    ).to(device)

    start_epoch = 0
    if Params.START_FROM_BEST and os.path.exists(Params.BEST_WEIGHT_PATH):
        # Note: only load if checkpoint matches DualStream architecture
        model.load_state_dict(torch.load(Params.BEST_WEIGHT_PATH, map_location=device))
...
if __name__ == "__main__":
    ...
    # Quick test: run test (ensure BEST_WEIGHT_PATH points to a DualStream checkpoint)
    test_best_model()
